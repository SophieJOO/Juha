...
export type CodexReportOptions = {
  days?: number;
  limit?: number;
  familyId?: string;
  generatedAt?: Date;
...
export async function buildCodexReport(
  prisma: PrismaClient,
  ...
  const familyFilter = options.familyId ? { familyId: options.familyId } : {};

  const families = await prisma.family.findMany({
    where: options.familyId ? { id: options.familyId } : undefined,
  ...
  const notes = await prisma.observationNote.findMany({
    where: { ...familyFilter, observedAt: { gte: since } },
  ...
  lines.push("");
  lines.push(`- 생성 시각: ${formatDate(generatedAt)}`);
  lines.push(`- 조회 범위: 최근 ${days}일`);
  lines.push(`- 조회 최대 기록 수: ${limit}`);
  lines.push(
  ...
  if (detailedNotes.length === 0) {
    ...
    for (const note of detailedNotes) {
      ...
      lines.push(`- 현재 판단: ${reviewLabel(detail.cueKey.review?.decision)}`);
